...
def save_games_to_pgn(games, output_directory, batch_size=100):
    ...
    for game in games:
        if game_count % batch_size == 0:
            if pgn_file:
                pgn_file.close()
            pgn_file_path = os.path.join(output_directory, f'structures_{batch_number}.pgn')
...
def main():
    input_pgn_file = 'structures.pgn'
    output_directory = './'

    with open(input_pgn_file, 'r', encoding='utf-8') as pgn_file:
